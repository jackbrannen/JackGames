...
export default function LobbyPage({ params }) {
  ...
  async function loadState() {
    const { data: gameData } = await supabase
      .from("cc_games")
      .select("code,phase,current_round")
      .eq("code", code)
      .single()
  ...
  useEffect(() => {
    const existing = localStorage.getItem(`cc:${code}:playerId`)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const { data: taken } = await supabase.from("cc_players").select("id").eq("game_code", code).ilike("name", saved.username.trim()).limit(1)
  ...
  async function startGame() {
    if (starting) return
    setStarting(true)
    const { error } = await supabase.rpc("cc_start_game", { p_code: code })
  ...
  async function handleInvite() {
    ...
    if (navigator.share) await navigator.share({ title: `Join Copycats — ${code}`, url })
  ...
  if (game?.phase !== "lobby" && !myPlayerId && game !== null) {
    return (
      <div style={{ minHeight: "100dvh", background: BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", textAlign: "center" }}>
        <div style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.18em", opacity: 0.5, marginBottom: 16, color: "white" }}>Copycats</div>
    ...
    <>
      <div style={{ paddingBottom: BOTTOM_PAD }}>
        <Lobby
          ...
          notFound={notFound}
          loading={!game}
          onRemovePlayer={async (id) => { await supabase.from("cc_players").delete().eq("id", id); loadState() }}
        />
      </div>
