...
-- Tables use a "cc_" prefix to avoid collisions.
-- ============================================================

-- Games (current_question_id added as FK after cc_questions is created)
...
-- Add FK now that cc_questions exists
...
create table public.cc_votes (
  ...
  answer_id   uuid not null references public.cc_answers(id) on delete cascade,
  created_at  timestamptz not null default now(),
...
-- ============================================================
-- RPC: cc_start_game
-- Copies player questions into cc_questions with random play_order,
...
begin
  ...
  -- Copy each player's question into cc_questions with a random play_order
...
begin
  ...
  set vote_count = (
    select count(*) from public.cc_votes where answer_id = p_answer_id
  )
