...
const POKE_COLORS = { dark: DARK, mid: MID, wl: WARM_LIGHT, yellow: GOLD, notifBg: "#0A1F35" }
...
export default function LobbyPage({ params }) {
  ...
  useEffect(() => {
    if (!replayOf || gamePhase !== "lobby" || myPlayerId || hasReplayJoinedRef.current || !gameExists) return
    const wasInParent = localStorage.getItem(`exquisitecorpse:${replayOf}:playerId`)
    ...
    ;(async () => {
      const { data: taken } = await supabase.from("ec_players").select("id").eq("game_code", code).ilike("name", saved.username.trim()).limit(1)
      if (taken?.length > 0) {
        localStorage.setItem(`exquisitecorpse:${code}:playerId`, taken[0].id)
  ...
  function onInvite() {
    ...
    if (navigator.share) navigator.share({ title: `Join Exquisite Corpse — ${code}`, url })
  ...
  if (gamePhase !== "lobby" && !myPlayerId) {
    return (
      <div style={{ minHeight: "100dvh", background: BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", textAlign: "center" }}>
        <div style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.18em", opacity: 0.5, marginBottom: 16, color: TEXT }}>Exquisite Corpse</div>
    ...
    <>
      <div style={{ paddingBottom: BOTTOM_PAD }}>
        <Lobby
          ...
          onRemovePlayer={async (id) => { await supabase.from("ec_players").delete().eq("id", id); loadState() }}
        />
      </div>
