...
const POKE_COLORS = { dark: DARK, mid: MID, wl: WARM_LIGHT, yellow: GOLD, notifBg: "#15111D" }
...
export default function LobbyPage({ params }) {
  ...
  useEffect(() => {
    const existing = localStorage.getItem(`mrwhite:${code}:playerId`)
  ...
  useEffect(() => {
    ...
    const wasInParent = localStorage.getItem(`mrwhite:${replayOf}:playerId`)
    ...
    ;(async () => {
      const { data: taken } = await supabase.from("mrwhite_players").select("id").eq("game_code", code).ilike("name", saved.username.trim()).limit(1)
      if (taken?.length > 0) {
        localStorage.setItem(`mrwhite:${code}:playerId`, taken[0].id)
  ...
  async function join() {
    ...
    const { data: existing } = await supabase
      .from("mrwhite_players").select("id").eq("game_code", code).ilike("name", trimmed).limit(1)
  ...
  function onInvite() {
    ...
    if (navigator.share) navigator.share({ title: `Join Mr. White — ${code}`, url })
  ...
  if (gamePhase !== "lobby" && !myPlayerId) {
    return (
      <div style={{ minHeight: "100dvh", background: BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", textAlign: "center" }}>
        <div style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.18em", opacity: 0.5, marginBottom: 16, color: TEXT }}>Mr. White</div>
    ...
    <>
      <div style={{ paddingBottom: BOTTOM_PAD }}>
        <Lobby
          ...
          onRemovePlayer={async (id) => { await supabase.from("mrwhite_players").delete().eq("id", id); loadState() }}
        />
      </div>
