...
/*
  ...
  Props:
    ...
    textColor      string     — title/input/Join-button text, default "white" (override on light bg colors)
    mutedTextColor string     — subtitle/Dummy-Game text, default "rgba(255,255,255,0.65)"
    errorColor     string     — error text, defaults to colors.yellow
...
export default function HomeScreen({
  ...
  isDummy = false,
  colors = {},
  textColor = "white",             // title, input text, Join button text — override on light bg colors
  mutedTextColor = "rgba(255,255,255,0.65)", // subtitle, Dummy Game label
  errorColor,                      // defaults to colors.yellow below — override when yellow-on-bg has poor contrast
}) {
  const { bg = "#111", wl = "#333", yellow = "#FBDF54" } = colors
  const resolvedErrorColor = errorColor ?? yellow
  ...
  return (
    <div style={{
      ...
      {!!error && (
        <p style={{ color: resolvedErrorColor, marginTop: 20, fontSize: 14, fontWeight: 600, textAlign: "center" }}>
          Error: {error}
        </p>
      )}
      ...
      <button
        ...
        style={{
          ...
          background: wl, color: mutedTextColor,
