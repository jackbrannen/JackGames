...
/*
  ...
  Props:
    ...
    players         { id, name, teamLabel?, teamColor? }[]
                               ...
                               into side-by-side colored team boxes
    teams           { label, color }[]  — fixes the team box columns (and their order) so an
                               as-yet-empty team still shows its own box instead of only
                               appearing once someone joins it. Omit to derive columns from
                               whichever teamLabels are present in players.
    ...
    textColor       hex | "white" (default "white") — this shell assumes a dark bg/dark
                                 by default (white text, light "rgba(255,255,255,X)" overlays).
                                 For a game whose bg/dark are themselves LIGHT (e.g. a light-base
                                 palette), pass a dark hex here instead — every text color and
                                 overlay derived below switches to match. Omit to keep today's
                                 look exactly as-is for every existing dark-themed game.
    surfaceOverlay  hex | "white" (default "white") — base color the semi-transparent button
                                 surfaces (How to Play, Settings cog, close ✕) are tinted from.
                                 Pass "black" (or any dark hex) alongside a dark textColor so
                                 those surfaces darken the light bg instead of lightening it.
...
// Converts "white"/"black"/a hex color into an rgba(...) string at the given
// alpha, so every existing rgba(255,255,255,X) text/overlay tint below can
// derive from textColor/surfaceOverlay instead of being hardcoded to white.
...
function PlayerRow({ p, myPlayerId, mid, onRemove, textColor }) {
  return (
    <div style={{ padding: "13px 16px", background: mid, display: "flex", alignItems: "center" }}>
      ...
      {onRemove && p.id !== myPlayerId && (
        <button
          ...
          style={{ background: "transparent", border: "none", color: withAlpha(textColor, 0.5), fontSize: 18, fontWeight: 800, cursor: "pointer", padding: "2px 4px", lineHeight: 1, flexShrink: 0 }}
        >
          ✕
        </button>
      )}
...
export default function Lobby({
  ...
  if (notFound) {
    return (
      <div style={{ minHeight: "100dvh", background: bg || dark, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ color: textColor, fontSize: FONT_SIZE.heading, fontWeight: FONT_WEIGHT.bold }}>Room not found.</p>
      </div>
    )
  }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100dvh", background: bg || dark, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ color: withAlpha(textColor, OPACITY.muted), fontSize: FONT_SIZE.bodyLg, fontWeight: FONT_WEIGHT.bold }}>Loading…</p>
      </div>
    )
  }

  return (
    <div style={{ minHeight: "100dvh", background: bg || dark, color: textColor }}>
      ...
      <div style={{ padding: `${SPACE.xl}px ${SPACE.lg}px ${SPACE.lg}px`, background: dark, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: SPACE.sm }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: SPACE.xs, flexShrink: 0, marginTop: 4, alignItems: "stretch" }}>
          {onInvite && (
            <button
              onClick={onInvite}
              style={{ background: wl, color: textColor, fontSize: FONT_SIZE.min, fontWeight: FONT_WEIGHT.heavy, padding: "10px 16px" }}
            >
              Invite
            </button>
          )}
          {howToPlayContent && (
            <button
              ...
              style={{ background: withAlpha(surfaceOverlay, 0.15), color: textColor, fontSize: FONT_SIZE.min, fontWeight: FONT_WEIGHT.heavy, padding: "10px 14px" }}
            >
              How to Play
            </button>
          )}
      ...
      {settingsContent && (
        <div style={{ padding: `${SPACE.lg}px ${SPACE.lg}px 0`, display: "flex", justifyContent: "flex-end" }}>
          <button
            ...
            style={{ background: withAlpha(surfaceOverlay, 0.15), color: textColor, fontSize: 24, fontWeight: FONT_WEIGHT.heavy, padding: "10px 14px", lineHeight: 1 }}
          >
            ⚙️
          </button>
        </div>
      ...
      {/* Player list */}
      <div style={{ padding: `${SPACE.xl}px ${SPACE.lg}px 0` }}>
        <div style={{ ...STYLE.sectionHeader, color: textColor, marginBottom: 4 }}>
          Players
        </div>
        ...
        {(() => {
          const hasTeams = !!teams || players.some(p => p.teamLabel)
          if (!hasTeams) {
            return (
              <div style={{ display: "flex", flexDirection: "column", gap: GAP.card }}>
                {players.map(p => <PlayerRow key={p.id} p={p} myPlayerId={myPlayerId} mid={mid} onRemove={onRemovePlayer} textColor={textColor} />)}
              </div>
            )
          }
          // Group by team, preserving insertion order — rendered as side-by-side colored
          // boxes (solid header bar per team) rather than stacked labels, so two teams read
          // as two distinct places rather than one continuous list. When `teams` is given,
          // its labels/order win (so an empty team still gets a box); otherwise derived from
          // whichever teamLabels are actually present.
          const teamOrder = []
          const teamMap = {}
          if (teams) {
            teams.forEach(t => { teamMap[t.label] = { color: t.color, players: [] }; teamOrder.push(t.label) })
          }
          players.forEach(p => {
          ...
          return (
            <div style={{ display: "grid", gridTemplateColumns: `repeat(${teamOrder.length}, 1fr)`, gap: GAP.card }}>
              {teamOrder.map(label => (
                <div key={label} style={{ background: mid, overflow: "hidden" }}>
                  <div style={{
                    background: teamMap[label].color || wl,
                    color: "white",
                  ...
                  <div style={{ padding: "8px 10px 10px", display: "flex", flexDirection: "column", gap: GAP.card }}>
                    {teamMap[label].players.length === 0 && (
                      <div style={{ fontSize: FONT_SIZE.small, opacity: OPACITY.muted, fontStyle: "italic", padding: "6px 2px" }}>No players</div>
                    )}
                    {teamMap[label].players.map(p => (
                      <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 2px" }}>
                        <span style={{ flex: 1, fontSize: FONT_SIZE.small + 1, fontWeight: FONT_WEIGHT.bold, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {p.name}
                          {p.id === myPlayerId && <span style={{ fontSize: 11, opacity: OPACITY.muted, marginLeft: 5 }}>you</span>}
                        </span>
                        {onRemovePlayer && p.id !== myPlayerId && (
                          <button
                            onClick={() => { if (typeof window !== "undefined" && window.confirm(`Remove ${p.name}?`)) onRemovePlayer(p.id) }}
                            aria-label={`Remove ${p.name}`}
                            style={{ background: "transparent", border: "none", color: withAlpha(textColor, 0.5), fontSize: 15, fontWeight: 800, cursor: "pointer", padding: "2px 4px", lineHeight: 1, flexShrink: 0 }}
                          >
                            ✕
                          </button>
                        )}
