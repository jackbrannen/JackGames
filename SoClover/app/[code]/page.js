...
const POKE_COLORS = { dark: DARK, mid: MID, wl: WARM_LIGHT, yellow: GOLD, notifBg: "#2E4510" }
...
export default function LobbyPage({ params }) {
  ...
  useEffect(() => {
    const existing = localStorage.getItem(`soclover:${code}:playerId`)
  ...
  async function join() {
    ...
    const { data: existing } = await supabase
      .from("soclover_players").select("id").eq("game_code", code).ilike("name", trimmed).limit(1)
  ...
  function onInvite() {
    ...
    if (navigator.share) navigator.share({ title: `Join So Clover — ${code}`, url })
  ...
  if (gamePhase !== "lobby" && !myPlayerId) {
    return (
      <div style={{ minHeight: "100dvh", background: BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", textAlign: "center" }}>
        <div style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.18em", opacity: 0.5, marginBottom: 16, color: TEXT }}>So Clover</div>
    ...
    <>
      <div style={{ paddingBottom: BOTTOM_PAD }}>
        <Lobby
          ...
          onRemovePlayer={async (id) => { await supabase.from("soclover_players").delete().eq("id", id); loadState() }}
        />
      </div>
