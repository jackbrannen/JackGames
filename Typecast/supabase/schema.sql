...
CREATE TABLE IF NOT EXISTS tc_matchups (
  ...
  used_words text[] NOT NULL DEFAULT '{}',   -- shuffled chosen words (the guess pool — extras hidden)
...
--   tc_submit_key(code, matcher, key[])   store a matcher's assignment; when ALL in -> phase=guess (first matchup)
