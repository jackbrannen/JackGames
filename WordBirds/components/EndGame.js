...
export default function EndGame({
  ...
  const { yellow = "#FBDF54", wl = "rgba(255,255,255,0.15)", primaryText = "#000", secondaryText = "white" } = colors
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "40px 24px", paddingBottom: bottomPad, animation: "endGameIn 300ms ease-out both" }}>
      ...
      <div style={{ marginTop: 40, display: "flex", flexDirection: "column", gap: 10 }}>
        {onPlayAgain && (
          <button
            onClick={onPlayAgain}
            style={{ background: yellow, color: primaryText, fontSize: 16, fontWeight: 900, padding: "14px 24px", width: "100%" }}
          >
            Play Again
          </button>
        )}
        <a
          ...
          style={{ background: wl, color: secondaryText, fontSize: 16, fontWeight: 700, padding: "14px 24px", width: "100%", display: "block", textAlign: "center", textDecoration: "none" }}
        >
          Play Another Game
        </a>
      </div>
