...
/*
  ...
  Props:
    ...
    colors          { dark, mid, wl, yellow, bg?, ink?, inkRgb? }
                                 — bg is the main page background (optional, defaults to dark).
                                 ink/inkRgb are for LIGHT-background games only (default "white"/
                                 "255,255,255" — byte-identical to prior behavior for every dark-bg
                                 game). They only affect the sections that sit directly on `bg`
                                 (player list, loading/not-found states, the settings-cog button) —
                                 the header, Settings modal, and How-to-Play modal always sit on a
                                 dark surface regardless of theme and are intentionally untouched.
...
function PlayerRow({ p, myPlayerId, mid, inkRgb, onRemove }) {
  return (
    <div style={{ padding: "13px 16px", background: mid, display: "flex", alignItems: "center" }}>
      ...
      {onRemove && p.id !== myPlayerId && (
        <button
          ...
          style={{ background: "transparent", border: "none", color: `rgba(${inkRgb},0.5)`, fontSize: 18, fontWeight: 800, cursor: "pointer", padding: "2px 4px", lineHeight: 1, flexShrink: 0 }}
        >
          ✕
        </button>
      )}
...
export default function Lobby({
  ...
  const { dark = "#333", mid = "#444", wl = "#555", yellow = "#FBDF54", bg, ink = "white", inkRgb = "255,255,255" } = colors
  ...
  if (notFound) {
    return (
      <div style={{ minHeight: "100dvh", background: bg || dark, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ color: ink, fontSize: FONT_SIZE.heading, fontWeight: FONT_WEIGHT.bold }}>Room not found.</p>
      </div>
    )
  }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100dvh", background: bg || dark, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ color: `rgba(${inkRgb},${OPACITY.muted})`, fontSize: FONT_SIZE.bodyLg, fontWeight: FONT_WEIGHT.bold }}>Loading…</p>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: "100dvh", background: bg || dark, color: "white" }}>
      ...
      {settingsContent && (
        <div style={{ padding: `${SPACE.lg}px ${SPACE.lg}px 0`, display: "flex", justifyContent: "flex-end" }}>
          <button
            ...
            style={{ background: `rgba(${inkRgb},0.15)`, color: ink, fontSize: 24, fontWeight: FONT_WEIGHT.heavy, padding: "10px 14px", lineHeight: 1 }}
          >
            ⚙️
          </button>
        </div>
      ...
      {/* Player list */}
      <div style={{ padding: `${SPACE.xl}px ${SPACE.lg}px 0`, color: ink }}>
        <div style={{ ...STYLE.sectionHeader, color: ink, marginBottom: 4 }}>
          Players
        </div>
        ...
        {(() => {
          ...
          if (!hasTeams) {
            return (
              <div style={{ display: "flex", flexDirection: "column", gap: GAP.card }}>
                {players.map(p => <PlayerRow key={p.id} p={p} myPlayerId={myPlayerId} mid={mid} inkRgb={inkRgb} onRemove={onRemovePlayer} />)}
              </div>
            )
          }
          ...
          // Exactly 2 teams (e.g. Boys vs Girls): side-by-side columns, each
          // headed by its own colored name bar, instead of a stacked list —
          // reads more clearly as "two teams" than one label after another.
          if (teamOrder.length === 2) {
            return (
              <div style={{ display: "flex", gap: 8 }}>
                {teamOrder.map(label => (
                  <div key={label} style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ background: teamMap[label].color || mid, color: "white", padding: "8px", textAlign: "center", fontSize: FONT_SIZE.min, fontWeight: FONT_WEIGHT.heavy, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                      {label}
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: GAP.card, marginTop: GAP.card }}>
                      {teamMap[label].players.map(p => <PlayerRow key={p.id} p={p} myPlayerId={myPlayerId} mid={mid} inkRgb={inkRgb} onRemove={onRemovePlayer} />)}
                    </div>
                  </div>
                ))}
              </div>
          ...
          return teamOrder.map(label => (
            <div key={label} style={{ marginBottom: 16 }}>
              <div style={{ fontSize: FONT_SIZE.min, fontWeight: FONT_WEIGHT.heavy, textTransform: "uppercase", letterSpacing: "0.12em", color: teamMap[label].color || `rgba(${inkRgb},${OPACITY.muted})`, marginBottom: 6 }}>
                {label}
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: GAP.card }}>
                {teamMap[label].players.map(p => <PlayerRow key={p.id} p={p} myPlayerId={myPlayerId} mid={mid} inkRgb={inkRgb} onRemove={onRemovePlayer} />)}
